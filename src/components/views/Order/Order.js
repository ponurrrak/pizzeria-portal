          ...
          <div key={index}>
            <ListItem className={styles.menuItem}>
              ...
              <ButtonGroup>
                ...
                <Button disabled={!isMenuActive} className={styles.orderBadge}>
                  <Badge
                    color={isMenuActive ? 'secondary' : 'primary'}
                    badgeContent={orderItem.amount}
                  >
                    <ShoppingCartIcon/>
                  </Badge>
                </Button>
...
const Order = ({match: {params: {id}}, location: {state}}) => {
  ...
  return (
    <Paper className={styles.component}>
      ...
      <Grid container
        spacing={3}
        justifyContent={(order.length && isMenuActive) ? 'flex-start' : 'center'}
      >
        {isMenuActive ?
